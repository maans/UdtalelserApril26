...
// Used for cache-busting verification in the UI.
  ...
  const BUILD_ID = 'v2.3-2026-04-24-print-table-logo-inline';
...
async function openPrintWindowForStudents(students, settings, title) {
  ...
  // v2.3: Brug altid det indlejrede lille PNG-logo i printvinduet.
  // Det undgår GitHub-mapper, langsom billedhentning og iPad/Safari-problemer med eksterne assets.
  const logoSrc = PRINT_HEADER_LOGO_DATAURL;
  ...
  const pagesHtml = list.map(st => {
    ...
    // Fjern kun den gamle tekst-signatur fra body'en. Den nye signatur er en HTML-table.
    ...
    ).toString().trim() || 'Stinne Krogh Poulsen';

    // Table er valgt med vilje: iPad/Safari printer flex-grid mere ustabilt.
    const signatureHtml = `
      <table class="signatureTable" role="presentation" aria-hidden="true">
        <tr>
          <td class="sigCell sigKontakt">
            <div class="sigName">${escapeHtml(kontaktNavn)}</div>
            <div class="sigLabel">Kontaktlærere</div>
          </td>
          <td class="sigSpacer"></td>
          <td class="sigCell sigForstander">
            <div class="sigName">${escapeHtml(forstanderNavn)}</div>
            <div class="sigLabel">Forstander</div>
          </td>
        </tr>
      </table>`;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .sigKontakt { width: 47%; }
  .sigSpacer { width: 10%; }
  .sigForstander { width: 43%; }
  .sigName { white-space: normal; overflow-wrap: anywhere; }
  .sigLabel { margin-top: 1.5mm; }
...
(function(){
  ...
  window.addEventListener('load', function(){
    ...
    if (!pending.length) return setTimeout(doPrint, 500);
    var left = pending.length;
    function done(){ if (--left <= 0) setTimeout(doPrint, 500); }
...
async function printAllKStudents() {
  ...
  const title = isAll ? 'Udtalelser v2.0 – print K-gruppe' : 'Udtalelser v2.0 – print K-elever';
...
kGroups.forEach(g => {
  ...
  const title = 'Udtalelser v2.0 – print alle K-grupper';
...
async function printAllStudents() {
  ...
  const title = 'Udtalelser v2.0 – print alle elever';
  await openPrintWindowForStudents(all, getSettings(), title, { preserveOrder: true });
}
