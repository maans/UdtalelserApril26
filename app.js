...
// Used for cache-busting verification in the UI.
  ...
  const BUILD_ID = 'v2.2-2026-04-24-signatur-logo-inline';
...
async function openPrintWindowForStudents(students, settings, title) {
  ...
  const pagesHtml = list.map(st => {
    ...
    // Signature block: build a real two-column HTML signature, never relying on the old space-aligned template line.
    // The old text signature is stripped from the body when detected, but the HTML signature is ALWAYS added.
    ...
    try {
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (hasForstander && hasKontakt) { cutStart = i; break; }
      }
      if (cutStart < 0) {
        for (let i = lines.length - 1; i >= 0; i--) {
          const ln = String(lines[i] || '').trim();
          if (/^forstander$/i.test(ln) || /forstander/i.test(ln)) { cutStart = i; break; }
        }
      }
      if (cutStart > 0) {
...
<head>
  ...
  <style>
    ...
    @media print{
      /* iPad/Safari klipper fixed/absolute footers i print-preview. Derfor ligger signaturen i normalt flow. */
      body.print-single .signatures,
      body.print-multi .signatures{ position: static; width: 100%; }
      body.print-multi .page{ position: relative; }
      body.print-multi .content{ position: relative; min-height: auto; }
      pre.statement{ padding-bottom: 0; }
    }
